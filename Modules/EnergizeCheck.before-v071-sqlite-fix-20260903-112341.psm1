...
function Set-ECDocumentEvidence {
    ...
    $req=Invoke-SqliteQuery -DataSource $DatabasePath -Query 'SELECT requirement_id,asset_id FROM document_requirements WHERE project_id=@p AND requirement_code=@c;' -SqlParameters @{p=$ProjectId;c=$RequirementCode}
    if($null -eq $req){throw "Document requirement '$RequirementCode' does not exist."}
    $exists=0;$fileName='';$hash=''
    if(-not [string]::IsNullOrWhiteSpace($FilePath)){
        $fileName=[IO.Path]::GetFileName($FilePath)
        if(Test-Path -LiteralPath $FilePath){
            $exists=1
            if(-not $SkipHash){$hash=(Get-FileHash -LiteralPath $FilePath -Algorithm SHA256).Hash}
        }
    }
    if([string]::IsNullOrWhiteSpace($ReceivedAt)){$ReceivedAt=(Get-Date).ToString('yyyy-MM-dd HH:mm:ss')}
    Invoke-SqliteQuery -DataSource $DatabasePath -Query @'
INSERT INTO document_evidence(project_id,requirement_id,asset_id,document_code,file_name,file_path,revision,is_current,file_exists,sha256,references_document_code,references_revision,received_at,updated_at)
VALUES(@p,@req,@asset,@doc,@name,@path,@rev,@current,@exists,@hash,@refdoc,@refrev,@received,datetime('now'))
ON CONFLICT(requirement_id) DO UPDATE SET asset_id=excluded.asset_id,document_code=excluded.document_code,file_name=excluded.file_name,file_path=excluded.file_path,revision=excluded.revision,is_current=excluded.is_current,file_exists=excluded.file_exists,sha256=excluded.sha256,references_document_code=excluded.references_document_code,references_revision=excluded.references_revision,received_at=excluded.received_at,updated_at=datetime('now');
'@ -SqlParameters @{p=$ProjectId;req=[int]$req.requirement_id;asset=$req.asset_id;doc=$DocumentCode;name=$fileName;path=$FilePath;rev=$Revision;current=if($IsCurrent){1}else{0};exists=$exists;hash=$hash;refdoc=$ReferencesDocumentCode;refrev=$ReferencesRevision;received=$ReceivedAt}|Out-Null
    $ev=Invoke-SqliteQuery -DataSource $DatabasePath -Query 'SELECT evidence_id FROM document_evidence WHERE requirement_id=@r;' -SqlParameters @{r=[int]$req.requirement_id}
    Invoke-SqliteQuery -DataSource $DatabasePath -Query 'UPDATE document_versions SET is_current=0 WHERE evidence_id=@e;' -SqlParameters @{e=[int]$ev.evidence_id}|Out-Null
    Invoke-SqliteQuery -DataSource $DatabasePath -Query @'
INSERT INTO document_versions(evidence_id,revision,file_name,file_path,sha256,is_current,created_at)
VALUES(@e,@rev,@name,@path,@hash,@current,datetime('now'))
ON CONFLICT(evidence_id,revision) DO UPDATE SET file_name=excluded.file_name,file_path=excluded.file_path,sha256=excluded.sha256,is_current=excluded.is_current;
'@ -SqlParameters @{e=[int]$ev.evidence_id;rev=$Revision;name=$fileName;path=$FilePath;hash=$hash;current=if($IsCurrent){1}else{0}}|Out-Null
    return [int]$ev.evidence_id
}
